...
## 2026-09-25 GitHub-site validation run (B-grade: mechanical)

- Command: `jevw run --goal "jev-ultrafast latest release" --url https://github.com/browser-use/jev-ultrafast --site github --escalate-cmd "python3 scripts/jevw-escaler.py"`
- Result: `status=done` (exit 0), 2 cycles, 1 escalation (done_verify), wall 8.5 s; receipts `bench/2026-09-25-github-site-run.jsonl` (CLICK synthetic rung 2 -> DONE ok).
- Zero site-specific code: `--site github` maps to the generic predicate (navigation cap only). First live proof the engine drives a second site unchanged.
- Honest caveat: the keyword gate is token-level — the run declared done on repo-page text satisfying >=2/3 goal tokens, not necessarily on release specifics. Gate precision, not engine generality, is the limiting factor here.
